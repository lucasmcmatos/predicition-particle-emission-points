...
SEQ_OUTPUT_PATH = "data/processed/complete_dataset.csv"
...
def build_dataset_completo():
    """
    Builds a complete dataset where each row corresponds to one timestamped measurement from a simulation.
    Appends the metadata and class label to each time step for sequence modeling (CNN, LSTM).

    Output:
        CSV file saved at SEQ_OUTPUT_PATH
    """
    ...
    with tqdm(total=total, desc="Building Complete Dataset") as pbar:
        for emission_point in all_emission_points:
            ...
            for tag in os.listdir(point_path):
                ...
                try:
                    df = pd.read_csv(devc_path, skiprows=[0])
                    df.columns = [col.strip() for col in df.columns]  # Clean column names
                    df = df.apply(pd.to_numeric, errors='coerce')

                    # Append metadata and class for each time row
                    meta = metadata_dict[key]
                    df["classe"] = emission_point
                    df["tag"] = tag
    ...
    os.makedirs(os.path.dirname(SEQ_OUTPUT_PATH), exist_ok=True)
    df_final.to_csv(SEQ_OUTPUT_PATH, index=False)
    print(f"Complete dataset saved to: {SEQ_OUTPUT_PATH}")

# Optional: call directly for testing
if __name__ == "__main__":
    build_dataset_completo()
